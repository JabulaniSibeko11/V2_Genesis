...
public class ObjectionFormService : IObjectionFormService
{
 ...
 Obj_Property_Info_AppealModel appeal)
    ...
    private async Task<ObjectionSubmitResult> SubmitAppealAsync(
        ...
        bool isMulti)
    {
        var originalObjectionNo =
            objAppeal?.Trim();

        if (string.IsNullOrWhiteSpace(originalObjectionNo))
        {
        ...
        // Link Appeal back to original Objection
        appeal.Obj_Ref =
            originalObjectionNo;

        // Applicant / Appeal type
        appeal.Appeal_Type =
            obj.Objector_Type;
        ...
        // ============================================================
        // 2. GENERATE / LOAD APPEAL NUMBER
        // ============================================================

        await db.Entry(appeal)
            .ReloadAsync();
        ...
        {
            appNo =
                $"{cfg.AppealPrefix}-{appId}";

            appeal.Appeal_No =
                appNo;

            db.Obj_Property_Info_Appeal.Update(
                appeal);
        ...
        // ============================================================
        // 3. POPULATE LEGACY / DASHBOARD APPEAL COLUMNS
        //
        // IMPORTANT:
        // These are the Municipal Valuer Decision values.
        //
        // No SQL injection:
        // ExecuteSqlInterpolatedAsync parameterises all values.
    ...
    ");
        ...
        _logger.LogInformation(
            ...
            "AppealNo={AppealNo}, ObjectionNo={ObjectionNo}, " +
            "Town={Town}, Category={Category}, MarketValue={MarketValue}",
            appNo,
            originalObjectionNo,
            obj2.Town_Name,
            obj6.Old_Category,
        ...
        // ============================================================
        // 4. COPY ORIGINAL OBJECTION PACK
        // ============================================================
        ...
        // PDF service uses obj as part of the appeal form.
        obj.Property_Type =
            appealPropertyType;
        ...
        // ============================================================
        // 7. SECTION 3 AGRICULTURAL
        // ============================================================

        objA3.Ref =
            appId;

        objA3.Appeal_Ref_SA3 =
            appId;

        objA3.Objection_Ref_SA3 =
            appNo;

        db.Obj_Section3Agri.Add(
            objA3);

        // ============================================================
        // 8. SECTION 3 BUSINESS
        // ============================================================

        objB3.Ref =
            appId;

        objB3.Appeal_Ref_SB3 =
            appId;

        objB3.Objection_Ref_SB3 =
            appNo;

        db.Obj_Section3Bus.Add(
            objB3);

        // ============================================================
        // 9. SECTION 3 RESIDENTIAL
        // ============================================================

        objR3.Ref =
            appId;

        objR3.Appeal_Ref_SR3 =
            appId;

        objR3.Objection_Ref_SR3 =
            appNo;

        db.Obj_Section3Res.Add(
            objR3);

        // ============================================================
        // 10. SECTION 4 BUSINESS
        // ============================================================

        objB4.Ref =
            appId;

        objB4.Appeal_Ref_SB4 =
            appId;

        objB4.Objection_Ref_SB4 =
            appNo;

        db.Obj_Section4Bus.Add(
            objB4);

        // ============================================================
        // 11. SECTION 4 RESIDENTIAL
        // ============================================================

        objR4.Ref =
            appId;

        objR4.Appeal_Ref_SR4 =
            appId;

        objR4.Objection_Ref_SR4 =
            appNo;

        db.Obj_Section4Res.Add(
            objR4);
        ...
        // 13. SECTION 6
        //
        // This is important because acknowledgement + submitted
        // Appeal PDF rebuild from these values.
        // ============================================================
        ...
        objFile.Objection_Ref_files =
            appNo;

        objFile.Evidence_count =
            count;

        objFile.Appeal_Ref_files =
            1;
        ...
        int fileCount, Obj_Files objFile)
    {
      ...
      .GenerateAcknowledgementAsync(acknowledgementData);
            ...
            // Section 49 belongs to the Objection submission only.
            // An Appeal must receive:
